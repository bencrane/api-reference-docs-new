...
This endpoint allows you to search against Prospeo's extensive database of 200M+ contacts using 20+ filters to build leads lists.

> **Important:** This endpoint does **not** return email or mobile. To enrich those, submit `person_id` from search results to the Enrich Person or Bulk Enrich Person endpoint.

Maximum results: 25,000 (1,000 pages × 25 results per page).

## How Are Credits Spent?

- **1 credit** per search request that returns at least one person.
...
1. Go to the dashboard and build a search with various filters.
2. Click `...` in the top-right corner of search results → click **API JSON**.
3. A modal opens with the current search payload. Use this to understand how filters map to the API.
...
Each result contains a `person` object and (if the person has a current job) a `company` object. Email and mobile are **not** included — use the Enrich Person endpoint.
...
`company_intent`, `person_socials`, `company.company_oids`, `company.company_list_oids`, `company.temp_matching_oids`
...
`company_location` and `person_location` take string values. Location strings must **exactly match** those available on the dashboard.
...
`person_job_title` takes string values.

**Programmatic access:** Use the Search Suggestions API to retrieve valid job title values (returns up to 25 titles ordered by popularity).
