# RegistrationRoleEdge

## Overview

A Relay edge containing a `RegistrationRole` and its cursor.

## Fields

| Field Name | Type | Arguments | Description |
|---|---|---|---|
| node | `Role` | — | The item at the end of the edge |
| cursor | `String!` | — | A cursor for use in pagination |
| id | `ID` | — | No description |
| registrationRecordedRoleId | `UUID` | — | No description |
| datasetIds | `JSON` | — | No description |
| firstObservedDate | `String` | — | No description |
| lastObservedDate | `String` | — | No description |
| rank | `Int` | — | No description |
| internalId | `String` | — | No description |
| internalRegistrationRecordedRoleId | `String` | — | No description |

## Interfaces Implemented

None

## Type Membership

- **Member of Edge(s):** None
- **Member of Connection(s):** `RegistrationRoleConnection`
- **Member of Union(s):** None
- **Referenced by Input(s):** None
- **Referenced by Object(s):** None

## Source

https://documentation.enigma.com/reference/graphql_api/objects/registration-role-edge
