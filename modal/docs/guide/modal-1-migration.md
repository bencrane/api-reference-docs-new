...
Going forward, it will be necessary to explicitly include the local dependencies of your Modal App. The easiest way to do this is with Image.add_local_python_source:
...
Setting target_inputs along with max_inputs may benefit performance by reducing latency during periods where the container pool is scaling up. See the input concurrency guide for more information.
...
Classes decorated with App.cls are no longer allowed to have a custom constructor (__init__ method). Instead, class parameterization should be exposed using dataclass-style modal.parameter annotations:
