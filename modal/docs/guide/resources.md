...
floating-point number of CPU cores: Note that this value corresponds to physical cores, not vCPUs. Modal also will set several environment variables that control multi-threading
...
maximum will be rejected with an [InvalidError](https://modal.com/docs/reference/modal.exception#modalexceptioninvaliderror). 
## Billing
For CPU and memory, you’ll be charged based on whichever is higher: your request or actual usage. Disk requests are billed by increasing the memory request at a 20:1 ratio. For example, requesting 500 GiB of disk will increase the memory request to 25 GiB, if it is not already set higher. 
## Resource limits
...
Above this limit, the host will begin to throttle the CPU usage of the container. You can alternatively set the CPU limit explicitly: 
### Memory limits
...
for the leaked GBs of memory. Specify this limit using the `memory` parameter on [@app.function()](https://modal.com/docs/reference/modal.App#function) or [Sandbox.create()](https://modal.com/docs/reference/modal.Sandbox#create): 
### Disk limits
Running Modal containers have access to many GBs of SSD disk, but the amount
of writes is limited by: The size of the underlying worker’s SSD disk capacity
...
Increased disk sizes can be requested with the `ephemeral_disk` parameter on [@app.function()](https://modal.com/docs/reference/modal.App#function) or [Sandbox.create()](https://modal.com/docs/reference/modal.Sandbox#create). The maximum
