...
Create from GitHub organization. Allows members in GitHub organization to auto-join the Workspace.

Create from scratch. You can invite anyone to your Workspace.
...
Users can automatically join a Workspace on their Workspace settings page if they are a member of the GitHub organization associated with the Workspace.
...
The user that creates a Workspace is automatically set as the Owner for that workspace. The owner can assign any other roles within the workspace, as well as remove other members of the workspace.
...
As an Owner or Manager you can administrate the access privileges of other members on the Workspace Management tab in settings.

NOTE

Modal supports Role-Based Access Control (RBAC) for more granular control over permissions at both the Workspace and Environment level.
