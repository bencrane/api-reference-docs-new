...
The official NVIDIA documentation explains Xid 13 has many potential causes, including hardware, driver, user app, system memory corruption, bus, thermal issue, and fb corruption.
NVIDIA Xid 13: GR: SW Notify Error
"NVIDIA Xid Errors" (PDF)
NVIDIA Xid Errors
NVIDIA GPU debug guidelines
DeepSeek AI paper explains Xid 13 indicates GPU memory anomalies, but can be a hardware error. Fire-Flyer AI-HPC: A Cost-Effective Software-Hardware Co-Design for Deep Learning (Aug 2024)
The official nvidia memory error doc does not mention Xid 13. NVIDIA GPU Memory Error Management
The official nvidia k8s device plugin implements Xid 13 as an application error. NVIDIA/k8s-device-plugin health check
The imbue-ai GPU health check implements Xid 13 as a non-hardware error.
The AWS support doc Troubleshoot Xid errors does not mention Xid 13.
The Google Cloud doc Troubleshoot GPU VMs explains Xid 13 is returned when applications have illegal memory access issues, and recommends cuda-memcheck and CUDA-GDB for debugging.
The Alibaba Cloud doc explains Xid 13 requires self-check by the user and users to resubmit the load to see if the issue goes away Diagnose GPU-accelerated nodes
The Tencent Cloud doc How to handle common Xid events does not mention Xid 13.
The Azure HPC GPU node problem detector does not implement any health check for Xid 13.
...
NVIDIA Xid 31: FIFO: MMU Error
NVIDIA Xid Errors PDF (September 2024)
NVIDIA Xid Errors
The official NVIDIA debugging guideline explains Xid 31 requires user application debugging.
The official NVIDIA memory error documentation does not mention Xid 31.
The official NVIDIA Kubernetes device plugin (v0.170.8) implements Xid 31 as a user application issue.
The imbue-ai GPU health check does not implement any Xid 31 health checks.
The AWS support doc Troubleshoot Xid errors does not mention Xid 31.
The Google Cloud troubleshooting guide explains Xid 31 is returned when applications have illegal memory access issues, and recommends cuda-memcheck and CUDA-GDB for debugging.
The Alibaba Cloud guide Diagnose GPU-accelerated nodes explains Xid 31 requires self-check by users and users to resubmit the workload to see if the same Xid is returned.
The Tencent Cloud doc How to handle common Xid events does not mention Xid 31.
The Azure HPC GPU node problem detector does not implement any Xid 31 health checks.
DeepSeek AI paper on Fire-Flyer explains Xid 31 is a user application issue, which may indicate the GPU memory anomalies, but can also be hardware issues.
...
The official NVIDIA docs explains Xid 32 is a DMA controller error which manages the communication between the NVIDIA driver and GPU over the PCI-E bus. This indicates PCI quality issues, not user application issues.
...
The official NVIDIA docs explains Xid 38 as a potential driver firmware error.
The Alibaba Cloud guide Diagnose GPU-accelerated nodes explains Xid 38 as a driver firmware issue.
...
The official NVIDIA docs explains Xid 43 as a user application hitting a software induced faults.
...
The official NVIDIA k8s device plugin (v0.17.0) implements Xid 43 as a user application error, indicating GPU stopped processing. The Alibaba Cloud guide explains Xid 43 as a user application error encountering a software induced fault, as a result, GPU stopped processing.
DeepSeek Fire-Flyer AI-HPC AI paper explains Xid 43 as a user application error, but may indicate GPU memory anomalies.
...
The official NVIDIA debugging guideline explains Xid 45 indicates channel affected by another failure, or may indicate unexpected Fabric Manager shutdown.
The official nvidia k8s device plugin implements Xid 45 as a user application error, being preemptive cleanup due to previous errors. NVIDIA/k8s-device-plugin health check, https://github.com/NVIDIA/k8s-device-plugin/blob/v0.17.0/internal/rm/health.go#L65-L71 (Aug 2024)
The AWS Troubleshoot Xid guide does not mention Xid 45.
The Google Cloud troubleshooting guide does not mention Xid 45.
The Alibaba Cloud guide explains Xid 45 may indicate multiple cuda applications hitting a DBE, or the result of application being stopped due to another error.
DeepSeek's Fire-Flyer AI paper explains Xid 45 indicates GPU memory anomalies, affecting code and data segments, but can be hardware-related.
...
The official NVIDIA docs explains Xid 69 as a potential hardware/driver issue.
NVIDIA Xid Errors
NVIDIA Xid Errors PDF (Sep 2024)
The official NVIDIA debugging guideline does not mention Xid 69.
The official NVIDIA Memory Error Management doc does not mention Xid 69.
The official NVIDIA k8s device plugin (v0.17.0) does not treat Xid 69 as an application error.
The AWS support document does not mention Xid 69.
The Google Cloud document does not mention Xid 69.
The Alibaba Cloud doc Diagnose GPU-accelerated nodes does not mention Xid 69.
The Tencent Cloud doc How to handle common Xid events does not mention Xid 69.
The Azure HPC GPU node problem detector) implements Xid 69 detection.
DeepSeek AI paper Fire-Flyer AI-HPC explains Xid 69 indicates GPU uncorrectable errors, recommending GPU reset or node reboot.
...
"NVIDIA Xid 121: C2C Link corrected error"
"NVIDIA Xid Errors"
"NVIDIA Xid Errors (PDF)
