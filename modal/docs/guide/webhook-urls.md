...
Modal Functions with the fastapi_endpoint, asgi_app, wsgi_app, or web_server decorator are made available over the Internet when they are served or deployed and so they have a URL.

This URL is displayed in the modal CLI output and is available in the Modal dashboard for the Function.

To determine a Function’s URL programmatically, check its get_web_url() property:
...
For deployed Functions, this also works from other Python code! You just need to do a from_name based on the name of the Function and its App:
...
Custom domains 
Custom domains are available on the Team and Enterprise plans. Visit workspace settings to upgrade.
...
NOTE
Adding a custom domain does not disable the auto-generated .modal.run URL. Both the custom domain and the original URL will continue to work.
