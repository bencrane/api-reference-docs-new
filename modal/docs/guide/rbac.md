...
RBAC is available on the Enterprise plan. Contact sales@modal.com for more information.
...
Enable RBAC: This requires an Enterprise plan. See our pricing page for more information.

Create a restricted Environment: Navigate to your Workspace’s Environment Management page in Settings and click Add Environment.

Configure Environment Roles: Once an Environment is restricted:
...
Manage access: Use the Manage button next to any restricted Environment to:
...
Proxy auth tokens for web endpoints 

Web endpoints can be protected with proxy auth tokens, which authenticate inbound HTTP requests before they reach your function.

On workspaces with RBAC enabled, proxy auth tokens are scoped — each token is explicitly associated with one or more Environments, and will only be accepted for web endpoints deployed to those Environments. This prevents a token intended for a staging endpoint from being used to call a production one.

Creating a scoped proxy auth token 
Navigate to Settings → Proxy Auth Tokens and click New Token.
...
Workspace-wide	Workspaces without RBAC	Any web endpoint in the workspace
...
Scoped proxy auth token used on a web endpoint in an associated Environment	Yes	Token must be explicitly associated with the target Environment
Scoped proxy auth token used on a web endpoint in a non-associated Environment	No	Token is not valid for Environments it has not been associated with
FAQ 
