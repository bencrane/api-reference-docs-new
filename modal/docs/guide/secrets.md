...
The most common way to create a Modal Secret is to use the [Secrets panel](https://modal.com/secrets) of the Modal dashboard, which also shows any existing Secrets.
...
You can then use your Secret by constructing it `from_name` when defining a Modal App and then accessing its contents as environment variables.
...
If you have `python-dotenv` installed, you can also use `Secret.from_dotenv()` to create a Secret from the variables in a `.env` file:
