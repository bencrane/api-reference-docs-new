...
Design your applications to be fault and preemption tolerant. Modal will send an interrupt signal to your container when preemption occurs. This will cause the Function’s exit handler to run, which can perform any cleanup within its grace period.
...
If you require Functions that are guaranteed not to be preempted, you may set the nonpreemptible parameter (available starting in client version v1.2.3) to True in the @app.function() or @app.cls() decorator. Note that a 3x multiplier will be applied to the list price for CPU and Memory usage when nonpreemptible is set to True.
