...
For reference documentation on the modal.Retries object, see this page.
...
By default, failures are propagated back to the caller. To treat exceptions like successful results and aggregate them in the results list instead, pass in return_exceptions=True.

When used with Function.map(), each input is retried independently.
