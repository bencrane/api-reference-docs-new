...
Source: [https://modal.com/docs/examples/mongodb-search](https://modal.com/docs/examples/mongodb-search)
...
Every day, we run a job on Modal that queries that STAC API
for new images of California and store the metadata in a MongoDB Atlas
database instance.
 
Asynchronously, we run a job on Modal to check which entries
...
We host a database client on Modal that allows the application’s
...
Finally, we run a simple static FastAPI server on Modal that serves
...
search endpoint is 
https://modal-labs-examples--clay-mongo-client-geo-search.modal.run/docs
.
